...
from preprocessed_whole_ds_comps.my_dataset import ProjectionDataloader
import math
...
def sentence_projection(
    ...
    for source_tag_ids, source_tag_type in zip(source_tags_ids, source_tags_type):
        ...
        if target_tag:
            ...
            print(f"#######################target tags idfs: {target_tags_ids}")
            print(f"#######################target tags idfs: {target_tags_types}")
    ...
    for target_tag_no, target_tag_ids in enumerate(target_tags_ids):
        ...
        # MERGE GROUPS WITH GAP = ~100

        i = 0
        while i < len(groups) - 1:
            if groups[i + 1][-1] - groups[i][0] <= 100:
        ...
        if target_tags_ids[i][-1] >= target_tags_ids[i + 1][0]:
            ...
            if target_tags_types[i] == target_tags_types[i + 1]:
                target_tags_ids[i] = sorted(
                    list(set(str(target_tags_ids[i]) + str(target_tag_ids[i + 1])))
                )
    ...
    i = 0
    while i < len(target_tags_ids) - 1:
        if int(target_tags_ids[i][-1]) >= int(target_tags_ids[i + 1][0]):
...
#rsource_tsv='/Users/anaryegen/Desktop/UPV/Master_Thesis/data/neoplasm/train.tsv'
#target_txt='/Users/anaryegen/Desktop/UPV/Master_Thesis/data/neoplasm/deepL/train_deepl.txt'
#alignments_talp='/Users/anaryegen/Desktop/talps_from_prj/deepl_neoplasm_train.awesome.test.talp'
#batch_size=32
#output = '/Users/anaryegen/Desktop/UPV/Master_Thesis/data/neoplasm/train_projection.tsv'
#print(dataset_projection(rsource_tsv,target_txt,alignments_talp, batch_size,output))

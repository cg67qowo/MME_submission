...
from utils.models import XLMRobertaForSequenceClassificationWeighted
from utils.metrics import compute_metrics
#from utils.tokenizer import ExtendedBertTokenizer
from torch.utils.tensorboard import SummaryWriter

from utils.models import CamemBertForSequenceTagging, XLMRobertaForSequenceTagging, BertForSequenceTagging, DistilBertForSequenceTagging
from utils.tokenizer import ExtendedBertTokenizer, ExtendedDistilBertTokenizer, ExtendedCamembertTokenizer, ExtendedXLMRobertaTokenizer
from torch.utils.tensorboard import SummaryWriter
...
MODEL_CLASSES = {
    ...
    "distilbert": (DistilBertConfig, DistilBertForSequenceTagging, ExtendedDistilBertTokenizer),
    #"auto-conf": (AutoTokenizer, AutoModel),
    "camembert": (CamembertConfig, CamemBertForSequenceTagging, ExtendedCamembertTokenizer),
    "xlm-roberta": (XLMRobertaConfig, XLMRobertaForSequenceTagging, ExtendedXLMRobertaTokenizer)
}
...
def train(args, train_dataset, model, tokenizer):
   ...
   # if not isinstance(train_dataset, Dataset):
    ...
    print(f"#######################CHECKING LENGTH f{len(train_dataset)}")
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()

    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)
    print("num_samples in sampler:", train_sampler.num_samples)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
          ...
          #  print("logits:", logits.shape)
            print("Labels shape:", batch[3].shape)
          ...
          #  loss = outputs.loss          # CrossEntropyLoss already computed if labels are provided
          #  logits = outputs.logits 
            labels = inputs["labels"]     # ground-truth targets

            # Print their shapes and a few values for debugging
           # print("Logits shape:", logits.shape)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                 ...
                 #   model.config.id2label = {0: "X", 1: "B-Claim", 2: "I-Claim", 3: "B-Premise", 4: "I-Claim", 5: "0"}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        eval_file_name = str(args.model_name_or_path) + "_" + str(args.max_seq_length) + "_" + str(args.per_gpu_train_batch_size) + "_" + str(args.learning_rate) + "_" + str(args.num_train_epochs)+ "_eval_results.txt"
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        size = os.path.getsize(cached_features_file)
    ...
    print(f"###############################TENSOR type: {type(features)}")
    print(f"###############################TENSOR len: {len(features)}")
    dataset = processor.features_to_dataset(features)
    print(f"###############################TENSOR type: {type(dataset)}")
    print(f"###############################TENSOR len: {len(dataset)}")
    for t in dataset.tensors:
        print(f"###############################TENSOR len: {(t.shape)}")
        print(t)
...
def main():
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
     ...
     #   model.config.architectures = ["BertForTokenClassification"]
        model.config.num_labels = 6
      #  model.config.id2label = {0: "X", 1: "B-Claim", 2: "I-Claim", 3: "B-Premise", 4: "I-Premise", 5: "0"}
      #  model.config.label2id = {v: k for k, v in model.config.id2label.items()}

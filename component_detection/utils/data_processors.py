...
class DataProcessor(object):
    ...
    def convert_examples_to_features(self, examples, max_seq_length, tokenizer,
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            input_mask += padding
            segment_ids += padding
            print(f"#############################LEN INPUT IDS 1 {len(input_ids)}", flush=True)
            print(f"#############################LEN INPUT MASK 1 {len(input_mask)}", flush=True)
            print(f"#############################LEN SEGMENT IDS  1{len(segment_ids)}", flush=True)

            if forSequenceTagging:
                label_ids = self.convert_labels_to_ids(labels)
                padding = [0] * (max_seq_length - len(label_ids))
                label_ids += padding
                print(f"#############################LEN INPUT IDS {len(input_ids)}", flush=True)
                print(f"#############################LEN INPUT MASK {len(input_mask)}", flush=True)
                print(f"#############################LEN SEGMENT IDS {len(segment_ids)}", flush=True)
                print(f"#############################LEN LABEL IDS {len(label_ids)}", flush=True)
                assert len(label_ids) == max_seq_length
              #  label_list = self.get_labels()
              #  label_map = {label: i for i, label in enumerate(label_list)}
               # label_ids = label_map[tuple(example.labels)]
    ...
    @classmethod
    def _read_conll(cls, input_file, token_column=0, label_column=1, replace=None):
...
class ArgMinSeqTagProcessor(DataProcessor):
    ...
    def convert_ids_to_labels(self, idx_list):
        ...
        print(f"########################    LIST OF LABELS: {labels_list}              ###########################")
        return labels_list
